...
- 正式 portfolio authority 是 PIOS 的唯讀 `PortfolioSnapshot` export。CI 可設定 `PIOS_PORTFOLIO_SNAPSHOT_PATH` 指向含 `snapshot_id`、`as_of`、`active_positions` 的私有 JSON；設定後讀取失敗只會 block 私人持股 brief，公開市場報告仍照常送出。
